    ...
    const provider: vscode.McpServerDefinitionProvider = {
        ...
        provideMcpServerDefinitions(): vscode.ProviderResult<vscode.McpServerDefinition[]> {
            ...
            if (isDevWorkspace && wsRoot) {
                ...
                outputChannel.appendLine(`[MCP] MedPaper: locked ${extensionVersion} / SDK2`);
            }
            ...
            if (assetAwareEntry && fs.existsSync(assetAwareEntry)) {
                ...
                [assetAwareCommand, assetAwareArgs] = buildPinnedUvxCommand(
                    uvPath,
                    assetAwarePin.packageSource,
        ...
        const handler: vscode.ChatRequestHandler = async (
            ...
            switch (request.command) {
                ...
                case 'help':
                    ...
                    stream.markdown('### 🧩 Prompt Workflows (`/mdpaper.*`)\n\n');
                    stream.markdown('以下 workflow prompts 會在執行 `MedPaper: Setup Workspace` 後隨 workspace prompt files 一起提供：\n\n');
                    stream.markdown('| 指令 | 說明 |\n');
                    stream.markdown('|------|------|\n');
                    stream.markdown('| `/mdpaper.search` | 文獻搜尋與探索 |\n');
                    stream.markdown('| `/mdpaper.concept` | 發展研究概念與 Novelty 驗證 |\n');
                    stream.markdown('| `/mdpaper.draft` | 撰寫草稿 |\n');
                    stream.markdown('| `/mdpaper.analysis` | 資料分析與 Table 1 / 圖表 |\n');
                    stream.markdown('| `/mdpaper.format` | 匯出 Word / Pandoc 文件 |\n');
                    stream.markdown('| `/mdpaper.project` | 專案建立與切換 |\n');
                    stream.markdown('| `/mdpaper.strategy` | 搜尋策略設定 |\n');
                    stream.markdown('| `/mdpaper.clarify` | 定點潤稿與修訂 |\n');
                    stream.markdown('| `/mdpaper.write-paper` | 11-Phase 完整寫稿流程 |\n');
                    stream.markdown('| `/mdpaper.literature-survey` | 系統性文獻調查 / corpus 建置 |\n');
                    stream.markdown('| `/mdpaper.manuscript-revision` | 稿件修改與回覆審稿意見 |\n');
                    ...
                    stream.markdown('- PubMed 搜尋優先 `unified_search(query)`；PICO / MeSH 再配 `parse_pico()` / `generate_search_queries()`\n');
                    stream.markdown('- manuscript 草稿前先做 `validation_action(action="concept", filename="concept.md")`，Novelty 需 ≥ 75\n\n');
