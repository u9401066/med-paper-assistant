...
class CGUState(BaseModel):
    ...
    creativity_level: CreativityLevel = Field(
        default=CreativityLevel.L1_COMBINATIONAL, description="創意層級"
    )
    target_count: int = Field(default=5, description="目標點子數量")
    constraints: list[str] = Field(default_factory=list, description="限制條件")

    # === 思考過程 ===
    current_mode: ThinkingMode = Field(default=ThinkingMode.REACT, description="當前思考模式")
    current_speed: ThinkingSpeed = Field(default=ThinkingSpeed.FAST, description="當前思考速度")
    thinking_steps: Annotated[list[ThinkingStep], add] = Field(
        default_factory=list, description="思考步驟歷史（可累加）"
    )

    # === 創意方法 ===
    selected_method: CreativityMethod | None = Field(default=None, description="選用的創意方法")
    method_context: dict = Field(default_factory=dict, description="方法特定上下文")

    # === 中間產物 ===
    raw_associations: Annotated[list[str], add] = Field(
        default_factory=list, description="原始聯想（可累加）"
    )
    candidate_ideas: Annotated[list[Idea], add] = Field(
        default_factory=list, description="候選點子（可累加）"
    )

    # === 輸出 ===
    final_ideas: list[Idea] = Field(default_factory=list, description="最終點子")
    ...
    # === 快思慢想策略 ===
    pattern: str = Field(
        default="explore", description="快慢模式: sprint(5:1), explore(3:1), refine(2:2), deep(1:3)"
    )
    ...
    def should_do_fast(self) -> bool:
        """是否該做快步驟"""
        return self.fast_steps_count < self.fast_target
    ...
    def is_cycle_complete(self) -> bool:
        """一個快慢週期是否完成"""
        return (
            self.fast_steps_count >= self.fast_target and self.slow_steps_count >= self.slow_target
        )
    ...
    def should_continue(self) -> bool:
        ...
        if self.iteration >= self.max_iterations:
            return False
        return not self.has_enough_ideas()
