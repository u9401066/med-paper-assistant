...
### 🆕 即將推出：Artifact-Centric Architecture

> 📋 [設計文件](docs/design/artifact-centric-architecture.md)

我們正在重新設計以支援**真正的非線性工作流程**：
...
| 功能 | 說明 |
|------|------|
| **`_workspace/` 暫存區** | 無專案也能儲存文獻、PDF、筆記 |
...
| `get_workspace_state` | **【新增】** 獲取工作區狀態（session 恢復用） |
| `sync_workspace_state` | **【新增】** 同步狀態供跨 session 使用 |
| `clear_recovery_state` | **【新增】** 清除恢復提示（成功恢復後） |
