...
# Draw.io MCP uses the pinned Python SDK2 integration.
...
if [ -d "$DRAWIO_FORK_DIR/src/drawio_mcp_server" ]; then
    ...
    echo "✅ pinned Draw.io MCP 2.0.0 / SDK2 will resolve through uvx"
fi
...
echo "  Otherwise it uses the exact archive commit in mcp-integration-lock.json; MCP1 fallbacks are disabled."
echo ""
...
echo "  🎨 drawio     — CONSORT/PRISMA flowcharts (pinned Python SDK2 runtime)"
echo "  📖 zotero     — Zotero reference import (pinned Keeper 2.1.0 / SDK2 via uvx)"
echo ""
