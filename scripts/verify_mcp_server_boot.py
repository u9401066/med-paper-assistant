...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify that the MCP server boots and registers expected assets.")
...
def main() -> int:
    ...
    print(
        "MCP server OK: "
        f"{tool_count} tools, {prompt_count} prompts, {resource_count} resources"
    )
    ...
    if resource_count != args.expected_resources:
        print(f"Expected {args.expected_resources} resources, got {resource_count}", file=sys.stderr)
        return 1
