...
|------|------|------|
| 自動偵測變數類型 | 連續 vs 類別 | 📋 |
| 正態性檢定 | 決定 mean±SD vs median(IQR) | 📋 |
| 分組比較 | t-test / Mann-Whitney / Chi-square / Fisher's | 📋 |
| 遺失值報告 | 自動標註遺失比例 | 📋 |
| 格式化輸出 | AMA / Vancouver / APA 風格 | 📋 |
...
│  檢查項目:                                                           │
│  ├── 數字一致性 (N=100 在各章節要一致)                              │
│  ├── 術語一致性 (不能 Introduction 用 A，Methods 用 B)              │
│  ├── 引用完整性 (Introduction 提到的文獻是否都在 References)        │
│  ├── 圖表引用 (Figure 1, Table 2 是否都有對應)                      │
│  └── 統計報告 (p 值格式、CI 格式是否一致)                           │
│                                                                      │
│  Output:                                                             │
│  ├── Consistency Report (JSON)                                       │
│  ├── Issue List (具體位置 + 修復建議)                               │
│  └── Auto-fix Suggestions (可一鍵修復的項目)                        │
└─────────────────────────────────────────────────────────────────────┘
```

### 4.3 Reviewer Response 生成器 ⭐⭐⭐⭐
...
├─────────────────────────────────────────────────────────────────────┤
│  Input:                                                              │
│  ├── reviewer_comments.md (審稿意見，可直接貼)                      │
│  ├── original_manuscript.md (原稿)                                   │
│  └── revised_manuscript.md (修改稿，可選)                           │
│                                                                      │
│  Output:                                                             │
│  ├── Response Letter (逐條回覆，格式化)                             │
│  ├── Track Changes Summary (修改摘要)                               │
│  ├── Rebuttal Suggestions (如何有禮貌地反駁)                        │
│  └── Missing Items Alert (未回應的 comment)                         │
└─────────────────────────────────────────────────────────────────────┘
```
