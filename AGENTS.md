...
| **C** post-manuscript | 全稿一致性、投稿清單、數量與交叉引用(C7擴展)、時間一致性                | `check_formatting`, `scan_draft_citations`, `read_draft` |
| **D** meta-learning   | SKILL + Hook 自我改進 + Review Retrospective(D7🆕)                      | `read_file`, `replace_string_in_file`                    |
