...
class PreAnalysisChecker:
    ...
    def check_concept(self, content: str) -> ChecklistResult:
        ...
        # 必要項目權重 70%，建議項目權重 30%
        required_score = (passed / len(self.REQUIRED_SECTIONS)) * 70 if failed < len(self.REQUIRED_SECTIONS) else 0
        optional_passed = passed - (len(self.REQUIRED_SECTIONS) - failed)
        optional_score = (optional_passed / len(self.RECOMMENDED_SECTIONS)) * 30 if len(self.RECOMMENDED_SECTIONS) > 0 else 0
