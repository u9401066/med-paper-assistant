    ...
    def to_report(self) -> str:
        ...
        if self.issues:
            ...
            for issue in self.issues:
                ...
                lines.append(f"| {issue.line_number} | `{issue.original}` | {issue.issue_type} | `{suggestion}` |")
...
ALL_WIKILINK_PATTERN = re.compile(
    r'\[\[([^\]]+)\]\]'
)

# PMID only 模式: [[12345678]] 或 [[PMID:12345678]]
PMID_ONLY_PATTERN = re.compile(
    r'\[\[(PMID:)?(\d{7,8})\]\]',
    re.IGNORECASE
)

# 混亂格式: Author 2024 [[12345678]]
MESSY_FORMAT_PATTERN = re.compile(
    r'([A-Z][a-z]+)\s*(\d{4})\s*\[\[(\d{7,8})\]\]',
    re.IGNORECASE
)
