...
class StandardizedReference:
    ...
    authors_full: Optional[List[Dict]] = None  # [{"last_name": "", "first_name": "", "initials": ""}]
    year: str = ""
...
class ReferenceConverter:
    ...
    def _from_zotero(self, article: Dict[str, Any]) -> StandardizedReference:
        ...
        authors, authors_full = self._convert_zotero_creators(
            article.get('creators', [])
        )
    ...
    def _convert_zotero_creators(self, creators: List[Dict]) -> tuple:
        ...
        for creator in creators:
            ...
            # Generate initials
            initials = ''.join(
                name[0].upper() for name in first_name.split() if name
            )
