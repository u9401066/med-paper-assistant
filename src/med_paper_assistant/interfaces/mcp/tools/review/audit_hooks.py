...
def _summarize_pending_evolutions(
    workspace_root: Path,
    slug: str,
) -> dict:
    ...
    return {
        ...
        "auto_apply_count": sum(1 for item in pending if item.auto_apply),
        "requires_confirmation_count": sum(1 for item in pending if not item.auto_apply),
        "items": [
            {
                "id": item.id,
                "type": item.type,
...
def _write_meta_learning_feedback(
    *,
    project_dir: Path,
    workspace_root: Path,
    slug: str,
) -> dict:
    """Persist meta-learning feedback where the next agent turn will see it."""
    feedback = _summarize_pending_evolutions(workspace_root, slug)
    feedback_path = project_dir / ".audit" / "meta-learning-feedback.yaml"
    feedback_path.parent.mkdir(parents=True, exist_ok=True)
    feedback_path.write_text(
        yaml.dump(feedback, default_flow_style=False, allow_unicode=True, sort_keys=False),
        encoding="utf-8",
    )
    try:
        wsm = get_workspace_state_manager()
        wsm.record_activity(
            tool_name="run_meta_learning",
            doing=f"Meta-learning complete for {slug}",
            next_action=(
                "Review .audit/meta-learning-feedback.yaml and apply safe pending evolutions"
                if feedback["pending_count"]
                else "Proceed with next pipeline gate"
            ),
            context=[
                f"pending_evolutions={feedback['pending_count']}",
                f"auto_apply={feedback['auto_apply_count']}",
...
def _write_pipeline_retrospective_artifact(
    *,
    project_dir: Path,
    slug: str,
    summary: str = "",
    ...
    """Create the Phase 10 pipeline-run artifact with required D7/D8 headings."""
    audit_dir = project_dir / ".audit"
    audit_dir.mkdir(parents=True, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d-%H%M")
    path = audit_dir / f"pipeline-run-{ts}.md"
    if path.exists():
        path = audit_dir / f"pipeline-run-{datetime.now().strftime('%Y%m%d-%H%M%S')}.md"

    state = get_workspace_state_manager().get_state()
    pipeline_state = state.get("pipeline_state", {})
    lines = [
        "---",
        "schema: mdpaper.pipeline_run.v1",
        f"project: {slug}",
        f"generated_at: {datetime.now().isoformat()}",
        "---",
        "",
        "# Pipeline Run Retrospective",
        "",
        "## Summary",
        "",
        summary.strip() or "Pipeline retrospective generated by MCP tooling.",
        "",
        "## Gate state snapshot",
        "",
        f"- Current phase: {pipeline_state.get('current_phase')}",
        f"- Last gate result: {pipeline_state.get('last_gate_result')}",
        f"- Next required action: {pipeline_state.get('next_required_action')}",
        "",
        "## D7 retrospective: Review Retrospective",
        "",
        d7_retrospective.strip()
        or "Review loop completed or is ready for final validation. See review-report-N.md and author-response-N.md artifacts.",
        "",
        "## D8 retrospective: EQUATOR Retrospective",
        "",
        d8_retrospective.strip()
        or "EQUATOR/reporting guideline compliance artifacts reviewed. See equator-compliance-N.md or equator-na-N.md.",
        "",
    ]
...
def register_audit_hook_tools(
    ...
    *,
    ...
    async def run_quality_audit(
        ...
        - quality-scorecard.yaml + quality-scorecard.md
    ...
    async def run_meta_learning(
        ...
        try:
            ...
            feedback = _write_meta_learning_feedback(
            ...
            lines.append(
                "files: .audit/meta-learning-audit.yaml,.audit/evolution-log.jsonl,.audit/meta-learning-feedback.yaml"
            )
            lines.append(
                f"pending_evolutions: {feedback['pending_count']} "
                f"(auto_apply={feedback['auto_apply_count']}, "
    ...
    async def run_writing_hooks(
        ...
        try:
            ...
            if "C10" in requested:
                r = engine.check_reference_fulltext_status(full_content)
                all_results["C10"] = r.to_dict()
                tracker.record_event("C10", "pass" if r.passed else "trigger")
                total_critical += r.critical_count
                total_warning += r.warning_count

            if "C11" in requested:
                r = engine.check_citation_distribution(full_content)
                all_results["C11"] = r.to_dict()
                tracker.record_event("C11", "pass" if r.passed else "trigger")
                total_critical += r.critical_count
                total_warning += r.warning_count

            if "C12" in requested:
                r = engine.check_citation_relevance_audit(full_content)
                all_results["C12"] = r.to_dict()
                tracker.record_event("C12", "pass" if r.passed else "trigger")
                total_critical += r.critical_count
                total_warning += r.warning_count

            if "C13" in requested:
                r = engine.check_figure_table_quality(full_content)
                all_results["C13"] = r.to_dict()
                tracker.record_event("C13", "pass" if r.passed else "trigger")
                total_critical += r.critical_count
                total_warning += r.warning_count
    ...
    @tool()
    async def write_pipeline_retrospective(
        summary: str = "",
        d7_retrospective: str = "",
        d8_retrospective: str = "",
        project: Optional[str] = None,
        ...
        """
        Write the Phase 10 `.audit/pipeline-run-*.md` retrospective artifact.

        This removes the previous agent friction where the gate required a
        timestamped filename and exact D7/D8 headings but provided no writer.
        """
        log_tool_call("write_pipeline_retrospective", {"project": project})
        try:
            project_info, workflow_error = resolve_project_context(
                project,
                required_mode="manuscript",
            ...
            slug = project_info["slug"]
            project_dir = Path(project_info["project_path"])
            path = _write_pipeline_retrospective_artifact(
                project_dir=project_dir,
                slug=slug,
                summary=summary,
            ...
            await report_tool_progress(ctx, 2, 2, "Pipeline retrospective written")
            rel = path.relative_to(project_dir).as_posix()
            log_tool_result("write_pipeline_retrospective", rel)
            return (
                "status: ok\n"
            ...
            return f"❌ Error writing pipeline retrospective: {e}"
    ...
    async def run_review_hooks(
        ...
        Call this during Phase 7 to verify review CONTENT quality,
        not just process compliance. Can be run standalone or as part
        of submit_review_round() (which calls these automatically).

        Available hooks:
        - R1: Review Report Depth (word count, perspectives, severity breakdown)
        - R2: Author Response Completeness (ACCEPT/DECLINE, ratio, evidence)
        - R3: EQUATOR Compliance Gate (checklist items ≥ 5)
        - R4: Review-Fix Traceability (ACCEPT'd issues → manuscript changes)
        - R5: Post-Review Anti-AI Gate (re-run A3/A3b after edits)
        - R6: Citation Budget Gate (refs ≤ journal limit, prioritisation)

        Args:
            round_num: Review round number (0 = auto-detect latest)
            hooks: Comma-separated hook IDs (e.g., "R1,R5,R6") or "all"
            issues_fixed: Number of issues fixed (for R4 traceability)
            project: Project slug (optional, uses current project)
        ...
        try:
            ...
            if round_num <= 0:
                ...
                for i in range(20, 0, -1):
                    if (audit_dir / f"review-report-{i}.md").is_file():
                        round_num = i
                        break
                if round_num <= 0:
                    return "❌ No review reports found. Start a review round first."

            # Read manuscript for R5/R6, including section-only draft workflows.
            ...
            if "R2" in requested:
                r = engine.check_author_response_completeness(round_num)
                results["R2"] = r.to_dict()
                total_critical += r.critical_count
                total_warning += r.warning_count

            if "R3" in requested:
                r = engine.check_equator_compliance(round_num)
                results["R3"] = r.to_dict()
                total_critical += r.critical_count
                total_warning += r.warning_count
            ...
            if "R5" in requested:
                if manuscript_content:
                    r = engine.check_post_review_anti_ai(manuscript_content)
                    results["R5"] = r.to_dict()
                    total_critical += r.critical_count
                    total_warning += r.warning_count
                else:
                    results["R5"] = {
            ...
            if "R6" in requested:
                if manuscript_content:
                    r = engine.check_citation_budget(manuscript_content)
                    results["R6"] = r.to_dict()
                    total_critical += r.critical_count
                    total_warning += r.warning_count
                else:
                    results["R6"] = {
            ...
            lines = [
                ...
                f"project: {slug}",
                f"round: {round_num}",
                f"hooks_run: {','.join(sorted(results.keys()))}",
                f"critical: {total_critical}",
                f"warnings: {total_warning}",
            ]
            for hook_id in sorted(results.keys()):
                hr = results[hook_id]
                lines.append(f"\n--- {hook_id} ---")
                lines.append(f"passed: {hr['passed']}")
                if hr.get("stats"):
