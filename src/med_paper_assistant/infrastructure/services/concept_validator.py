...
- Validation is MANDATORY before any concept file processing
- LLM scoring uses 3 rounds for reliability
- All 3 rounds must score 75+ to pass novelty check
...
class ConceptValidator:
    ...
    def validate(
        ...
        result = self._validate_structure(content, result)
    ...
    def validate_structure_only(self, file_path: str) -> ValidationResult:
    ...
    def _validate_structure(self, content: str, result: ValidationResult) -> ValidationResult:
        """
        ...
        for section_key, section_def in self.SECTION_DEFINITIONS.items():
            ...
            # Find section
            section_content = self._extract_section(content, section_def["patterns"])
            ...
            if section_content:
                ...
                if section_key == "selling_points":
                    ...
                    points = re.findall(r'Selling Point \d+.*?:.*?\S', content, re.IGNORECASE)
                    non_placeholder_points = [p for p in points if '[' not in p]
                    check.has_content = len(non_placeholder_points) >= section_def.get("min_count", 3)
                    
                    if not check.has_content:
                        result.errors.append(
                            f"{check.name}: Found {len(non_placeholder_points)} points, need at least 3"
                        )
                else:
                    min_words = section_def.get("min_words", 10)
                    check.has_content = word_count >= min_words
                    
                    if not check.has_content and section_def["required"]:
                        result.errors.append(
    ...
    def _evaluate_novelty(self, content: str, result: ValidationResult) -> ValidationResult:
        ...
        novelty_content = result.sections.get("novelty_statement", SectionCheck(name="", found=False, has_content=False)).content
    ...
    def _heuristic_novelty_score(self, content: str, round_num: int) -> int:
        ...
        # Positive indicators
        novelty_keywords = [
            "first", "novel", "unique", "new", "innovative", "original",
            "unprecedented", "pioneering", "breakthrough", "首次", "創新", "獨特"
        ]
        specificity_patterns = [
            r'\d+%', r'\d+\s*(patients|subjects|cases|participants)',
            r'compared to', r'unlike', r'in contrast'
        ]
        ...
        # Negative indicators
        vague_phrases = [
            "improved", "better", "enhanced", "optimized",
            "更好", "改善", "提升"
        ]
        for phrase in vague_phrases:
            if phrase.lower() in content_lower and not any(
                re.search(rf'{phrase}\s+\d+', content, re.IGNORECASE) 
                for _ in [1]
            ):
    ...
    def _check_consistency(self, content: str, result: ValidationResult) -> ValidationResult:
        ...
        novelty = result.sections.get("novelty_statement", SectionCheck(name="", found=False, has_content=False)).content
        gap = result.sections.get("research_gap", SectionCheck(name="", found=False, has_content=False)).content
        selling = result.sections.get("selling_points", SectionCheck(name="", found=False, has_content=False)).content
        outcomes = result.sections.get("expected_outcomes", SectionCheck(name="", found=False, has_content=False)).content
        ...
        if not result.consistency_passed:
            result.warnings.append(
                f"Consistency check: Score {consistency_score}/100 (need 70+)"
            )
    ...
    def _get_cached_result(self, file_path: str) -> Optional[ValidationResult]:
        ...
        if file_path in self._validation_cache:
            cached = self._validation_cache[file_path]
    ...
    def must_validate_before_processing(self, file_path: str) -> Tuple[bool, str]:
        """
