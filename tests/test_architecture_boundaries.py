...
def test_infrastructure_layer_does_not_import_application_or_interfaces() -> None:
    root = Path(__file__).resolve().parents[1] / "src" / "med_paper_assistant" / "infrastructure"
    violations = [
        f"{path.relative_to(root)} -> {module}"
        for path, module in _imports_under(root)
        if module.startswith(
