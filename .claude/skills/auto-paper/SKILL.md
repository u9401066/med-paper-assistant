...
1. **Phase 轉換**：Agent MUST call `pipeline_action(action="validate_phase", phase=N)` 且收到 PASS，才能開始 Phase N+1
2. **Phase 7 Review Loop**：Agent MUST call `pipeline_action(action="start_review")` 開始、`pipeline_action(action="submit_review", scores=...)` 結束。不可跳過輪次
3. **Pipeline 完成**：Agent MUST call `pipeline_action(action="heartbeat")` 確認 completion = 100% 才能宣稱完成
...
6. **Phase > 1 自動前提檢查**：`pipeline_action(action="validate_phase", phase=...)` 在 Phase > 1 時會自動檢查前置 Phase 的關鍵 artifacts（WARNING 級別）
7. **審計數據強制**：Phase 6 gate 不只檢查報告檔案存在，還驗證 quality-scorecard.yaml（≥4 維度、avg > 0）和 hook-effectiveness.yaml（≥1 hook 有事件記錄）
8. **自我改進數據強制**：Phase 10 gate 驗證 meta-learning-audit.yaml 有完整分析記錄（analysis_steps D1-D9、adjustments_count、lessons_count、suggestions_count 與 count/list 一致性），並產出 meta-learning-feedback.yaml 供下一輪 Agent 直接接續
...
- 所有 gate tools（`pipeline_action(action="validate_phase")`, `pipeline_action(action="heartbeat")`, `pipeline_action(action="start_review")`, `pipeline_action(action="submit_review")`）
...
**目的**：在任何寫作開始前，先登記用戶提供的原始素材，再建立結構化的期刊約束 + Pipeline 行為設定。
**產出**：`projects/{slug}/journal-profile.yaml` + `projects/{slug}/.audit/source-materials.yaml`
...
1. `project_action(action="current")` → 有專案就切換，沒有就建立
2. `project_action(action="create", name, paper_type)` 或 `project_action(action="switch", slug)`
3. `project_action(action="setup")`
...
3. 以 `read_session(action="pmids")` 取回 PMID，並以 PMID/DOI 去重
...
3. `draft_action(action="write", filename="concept.md", content=..., skip_validation=True)`
4. `validation_action(action="concept")(project=...)`
5. IF score < 75:
   - 自動修正 1 次 → 再驗證
...
1. `draft_action(action="sync_references", filename="manuscript.md")` → 生成 References section
...
1. `export_document(action="docx", draft_filename="manuscript.md", output_filename="paper.docx")` → 產出 DOCX
2. `export_document(action="pdf", draft_filename="manuscript.md", output_filename="paper.pdf")` → 產出 PDF
3. `inspect_export(action="list_templates")` → 選擇 template（優先匹配 journal-profile.journal.name）
4. `export_document(action="session_start")` → `export_document(action="session_insert")` × N
...
**目的**：確認 paper 交付物完整可審計。Git 僅作為 optional provenance，不可阻擋 paper-only workflow。
**觸發**：Phase 10 完成後自動進入。
...
1. 確認 Phase 9 `.docx` + `.pdf` 已產出
2. 確認 Phase 10 `.audit/pipeline-run-*.md`、`.audit/meta-learning-audit.yaml`、`.audit/meta-learning-feedback.yaml` 完整
...
B7a/B7b 遺漏為 WARNING（1 round `draft_action(action="patch")` 修正），B7d 為 WARNING，B7e 為 WARNING。
...
4. 不正確 → `draft_action(action="patch")` 更新為實際狀態
...
- [ ] Phase 0: source-materials.yaml 已產生；所有 pending_asset_aware 檔案已交給 asset-aware 或有明確 fallback reason
...
- [ ] .audit/pipeline-run-{YYYYMMDD-HHmm}.md 涵蓋所有 Phase，且含 `## D7 retrospective:` + `## D8 retrospective:`
- [ ] meta-learning-feedback.yaml 已生成，pending evolutions 已回灌到 workspace state
...
- [ ] .memory/ 已更新
- [ ] Hook D meta-learning 已執行（含 D7 Review + D8 EQUATOR + D9 Tool Telemetry Retrospective）
