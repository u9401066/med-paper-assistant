...
**Gate**: `pipeline_action(action="validate_phase", phase=9)` — docx + pdf 必須存在（CRITICAL）+ 必要文件清單完成
...
3. `git push` → 推送到遠端
4. `pipeline_action(action="validate_phase", phase=11)` → 驗證 PASS

**Gate**: `pipeline_action(action="validate_phase", phase=11)` — 工作目錄乾淨 + 已推送
